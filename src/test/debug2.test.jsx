...
test('test 1 - loading resolves', async () => {
  render(<Admin />)
  await waitFor(() => {
    const skeletons = document.querySelectorAll('.animate-pulse')
    if (skeletons.length > 0) throw new Error('still loading, skeletons: ' + skeletons.length)
  }, { timeout: 5000 })
})

test('test 2 - loading resolves again', async () => {
  render(<Admin />)
  await waitFor(() => {
    const skeletons = document.querySelectorAll('.animate-pulse')
    if (skeletons.length > 0) throw new Error('still loading, skeletons: ' + skeletons.length)
