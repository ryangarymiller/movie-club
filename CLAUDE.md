...
### Scheduling (4c — auto-activation ON via pg_cron; deadline enforcement still deferred)
**Auto-activation is now server-scheduled.** `pg_cron` runs `public.cron_auto_activate_due_months()` hourly (`'5 * * * *'`, job `auto-activate-due-months`): it activates any month with `auto_activate = true` + `status='upcoming'` whose `active_date` has arrived in US Pacific (DST-safe; the due-check is evaluated in `America/Los_Angeles`). It calls `activate_month` (which runs without a JWT in cron context — its caller-auth block is skipped — and enforces the single-active invariant + materialize + next-upcoming + readjustment). It is **inert until an admin toggles `auto_activate` on a month** (all default false), so enabling it changed no behavior. The client-soft `autoActivateDueMonths()` on This Month load remains as a redundant trigger.

**Deadline enforcement / auto-reveal is still intentionally deferred** — deadlines remain display-only. (Enabling it now would immediately force-reveal the active month, whose film deadlines have all passed.) The spec's intended model is a *soft* deadline: it triggers reveal + marks non-scorers absent (not zero), but late scores are still accepted and recalculate — not a hard submission lock.
