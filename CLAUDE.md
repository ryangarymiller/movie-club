...
- **Live reveal bus:** `movies` can't be in the realtime publication (postgres_changes payloads ignore column-level grants → would leak `picked_by_user_id` pre-reveal). Instead a `broadcast_reveal()` trigger sends a MINIMAL `{table, id}` payload on the public `'reveals'` broadcast topic when `scores_revealed`/`picker_revealed`/`veto_resubmit_required` flips or a month's `status` changes — covering the server-side pg_cron paths no client initiated. `<RevealBus>` (App.jsx, from `src/lib/useRevealRefresh.js`) holds the single channel subscription and re-dispatches a `mc-reveal` window event; Home/Films/ThisMonth refetch via `useRevealRefresh(cb)` and Stats/Awards via a `useRevealTick()` dep. Clients refetch through normal RLS-gated queries, so nothing sensitive rides the broadcast.
...
- **Rolling visibility (anonymity):** the discussion is gated like scores — you can't read others' reviews/comments on an active-month film until you've **scored** it (or it's revealed). The `reviews`/`comments` SELECT RLS enforces this server-side (visible iff author=you OR `scores_revealed` OR admin OR you've-scored-and-the-author-has-scored). The film overlay **also gates the display client-side on `canSeeScores`** (no admin bypass), so it locks the thread behind a "Submit your score to unlock the discussion" placeholder even for admins — fixing a leak where an admin (a playing member) could read reviews before scoring. The per-member scores list is gated the same way (`visibleRatings`/`scoresMessage`).
- **Live updates:** `CommentThread` subscribes to `reviews`/`comments`/`reactions`/`votes` realtime (reviews/comments filtered to the film) so a new post or reaction appears with no refresh; the discussion tables are in the `supabase_realtime` publication (RLS still scopes delivery per-subscriber). Home "Recent Activity" subscribes to `ratings`/`reviews`/`comments` for the same reason.
...
**Picks are TMDB-enriched at pick time.** The pick flow fetches `/movie/{id}?append_to_response=credits` and stores the FULL metadata in `upcoming_picks.metadata` (director, year, runtime, genre, plot, streaming providers, full cast, writers, TMDB vote stats); `activate_month` copies it all into the `movies` row at materialization (`coalesce` keeps existing values when a pick's metadata lacks a field). So films arrive with Cast & Crew / Connection Web / recap-plot data already populated — no post-activation backfill needed. The pick-change approval and veto-resubmit swap paths populate the same fields. Safety net: the Admin dashboard's **TMDB Metadata Backfill** (superseded the genre-only backfill) auto-runs when any film is missing genre/director/plot/year/runtime/cast/writers and fills ONLY the missing fields (never overwrites manual edits).
...
Films open the film overlay and member names/avatars open the **member overlay** throughout the app — Home, Stats, Awards, Films, the film overlay. (Direct `/profile/:id` navigation is preserved as a deep-link fallback.) Home "all caught up" box links to that month's scores (Films History); Home stats link to the relevant film or Films page. Home includes a "Club Members" browse strip and a collapsible Recent Activity section. **"Your Turn" and This Month's films are in watch order** (earliest `scoring_deadline` first). **Recent Activity masks others' score values** until the viewer can see that film (rolling): it always shows "X scored [film]" but only shows the number once `scores_revealed` or you've scored it — so an admin (who receives others' rating rows via the RLS bypass) doesn't leak unrevealed scores there. The This-Month **poster row** shows each film's **club average** as a badge prefixed with a 👥 glyph (with a "👥 = club average" legend) — visible per the rolling model (only once you've scored that film); the **"Your scores"** list below it shows your own submitted scores in the accent colour. The two are labelled distinctly so a club average is never mistaken for your own score.
...
- **TMDB Metadata Backfill** runs on the Admin dashboard when any film is missing genre, director, plot, year, runtime, cast, or writers (fetches details+credits from TMDB; fills only the missing fields, never overwriting manual edits). Mostly a safety net now — picks made in-app arrive already enriched (see Pick → Film Lifecycle).
...
**Prompt fidelity (watch order + names + film context):** the prompt lists films in **watch order** (the `movies` query orders by `scoring_deadline`, numbered `1..n`) and instructs the model to honor it — only film 1 "opened" the month and only the last "closed" it, no false "bookends" — fixing a recap that once called the *last* film the month's kickoff. Members are referred to by first name, except the function detects shared first names (the two Ryans — Ryan Miller / Ryan Bey) and instructs the model to disambiguate them by full/last name. **Each film also passes its `plot_summary`** (the stored TMDB blurb) into the prompt as an `about:` line, and the model is told to briefly weave in what a film is *about* (premise/themes/director) where it adds colour — a phrase or sentence, no plot dumps, no spoilers — drawing on that summary + its own knowledge of these well-known films, while NOT inventing club/score facts. So recaps read with movie context, not just numbers. The recap also surfaces **read-only on the Films → History tab** (below the film grid) for **revealed months only** — the active month's recap names pickers, so `HistoryTab` only fetches recaps for `status='revealed'` months (`RecapProse` is exported from `MonthReveal` and reused).
